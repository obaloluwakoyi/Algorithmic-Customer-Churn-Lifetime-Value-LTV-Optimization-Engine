...
| **Inferential A/B T-Test** | **T = 29.77, p = 1.24e-186** | Statistically proved churned clients are significantly older (~44.8 yrs vs 37.4 yrs). |
...
1. **Sure Things (4,784 accounts):** Customers who stay regardless of getting an offer. *Withheld $50 offer -> Saved $239,200.*
2. **Do Not Disturb / Low Impact (4,210 accounts):** Low baseline risk or negligible balance return.
3. **Lost Causes (892 accounts):** High churn probability unaffected by $50 offer. *Withheld offer -> Saved $44,600.*
...
* `Churn_Predictions_Output.csv`: Dataset with predicted P(Churn) scores and LTV estimates.
* `Targeted_Marketing_Campaign_List.csv`: Actionable list filtered by positive Expected Value (EV > $0).
